...
public class CPHInline
{
    ...
    private const int TOTAL_FABRICATORS = 3;
        ...
        Dictionary<int, List<string>> deputiesBySector)
    {
        ...
        // Sector 3 uses an explicit numeric boss flag. Treat its persisted
        // Main.Sector3.Boss value as the final authority so an old stage or
        // MIICBM value can never keep the defeated boss on the website.
        int bossAlive = sector == 3
            ? (
                GetGlobalInt(
                    prefix + ".Boss",
                    1
        ...
        {
            objectiveName = "Fabricators";
            objectiveTotal = TOTAL_FABRICATORS;
            objectiveRemaining = fabricators;
        }
        ...
        Dictionary<string, SectorSnapshot> sectors)
    {
        ...
        text.Append(" | S4 Synths ");
        text.Append(
            sectors["4"].synthsEnabled == 1
                ? "ACTIVE"
                : "OFF"
        );
    ...
    private class SectorSnapshot
    {
        ...
        public int fabricators { get; set; }
        public int synthsEnabled { get; set; }
